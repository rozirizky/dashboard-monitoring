...
class NewsSourceService:
    ...
    async def get_all(
        ...
        if search:
            expr = NewsSource.source.ilike(f"%{search}%")
            query = query.where(expr)
            count_query = count_query.where(expr)

        if category:
            expr = NewsSource.category == category
            query = query.where(expr)
            count_query = count_query.where(expr)

        if enabled is not None:
            expr = NewsSource.status == enabled
            query = query.where(expr)
            count_query = count_query.where(expr)
        ...
        result = await db.execute(
            query.order_by(NewsSource.priority.asc()).offset(offset).limit(size)
        )
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, source_id: int) -> NewsSource | None:
        result = await db.execute(
    ...
    @staticmethod
    async def get_source_by_priority(db: AsyncSession) -> list[NewsSource]:
        result = await db.execute(
            select(NewsSource)
    ...
    @staticmethod
    async def create(db: AsyncSession, data: NewsSourceCreate) -> NewsSource:
    ...
    async def update(
        ...
        await db.execute(
            update(NewsSource).where(NewsSource.id == source_id).values(**payload)
        )
        await db.flush()
        return await NewsSourceService.get_by_id(db, source_id)
