...
def _log(session, source: str, ok: bool, rows: int, ms: int, err: str | None = None):
    session.add(FetchLog(source=source, success=ok, rows_saved=rows, duration_ms=ms, error_msg=err))
    session.commit()
...
def _get_stock_watchlist() -> dict[str, list[str]]:
    ...
    try:
        ...
        for r in rows:
            group = (
                "Indonesia" if r.symbol.endswith(".JK")
...
def _get_forex_pairs() -> dict[str, str]:
    ...
    try:
        ...
        if not rows:
            return FOREX_PAIRS
        return {
            r.symbol: r.symbol.replace("=X", "").replace(r.symbol[:3], r.symbol[:3] + "/")
            for r in rows
        }
    finally:
        session.close()
    ...
    try:
        ...
        rows = [
            CryptoTrending(
                ...
                price_usd=prices.get(item["id"], {}).get("usd"),
                change_24h=prices.get(item["id"], {}).get("usd_24h_change"),
                volume_24h=prices.get(item["id"], {}).get("usd_24h_vol"),
            )
        ...
        log.info("[%s] %d rows (%dms)", src, len(rows), ms)
    except Exception as e:
        session.rollback()
        ms = int((time.monotonic() - t0) * 1000)
        _log(session, src, False, 0, ms, str(e))
        log.error("[%s] Error: %s", src, e)
    finally:
        session.close()
    ...
    try:
        ...
        for ticker, group in all_pairs:
            try:
                ...
                log.warning("Ticker skip [%s]: %s", ticker, e)

        session.bulk_save_objects(rows)
        session.commit()
        ms = int((time.monotonic() - t0) * 1000)
        _log(session, src, True, saved, ms)
        log.info("[%s] %d/%d tickers (%dms)", src, saved, len(all_pairs), ms)
    except Exception as e:
        session.rollback()
        ms = int((time.monotonic() - t0) * 1000)
...
def _forex_yahoo(now: datetime, pairs: dict[str, str]) -> list[ForexRate]:
    ...
    for ticker, label in pairs.items():
        try:
            ...
            log.warning("Forex skip [%s]: %s", ticker, e)
    return rows
...
def fetch_forex():
    ...
    try:
        ...
        if len(rows) < 5:
            log.warning("Yahoo forex insufficient data, falling back to er-api")
        ...
        ms = int((time.monotonic() - t0) * 1000)
        _log(session, src, True, len(rows), ms)
        log.info("[%s] %d pairs (%dms)", src, len(rows), ms)
    except Exception as e:
        session.rollback()
        ms = int((time.monotonic() - t0) * 1000)
        _log(session, src, False, 0, ms, str(e))
...
def cleanup_old_data(hours: int = DATA_RETENTION_HOURS):
    ...
    try:
        for Model in [CryptoTrending, CryptoMarket, StockQuote, ForexRate]:
            ...
            session.commit()
            log.info("[cleanup] %s: %d rows deleted", Model.__tablename__, result.rowcount)
    except Exception as e:
        session.rollback()
...
def run_all():
    log.info("Starting full market data fetch...")
    for label, fn in [
