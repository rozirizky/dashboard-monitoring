...
async def list_news_sources(
    ...
    sources, total = await NewsSourceService.get_all(
        db=db, page=page, size=size, search=search, category=category, enabled=enabled
    )
    pages = NewsSourceService.calculate_pages(total, size)
    return ResponseSchema(
        message="Berhasil mengambil data news sources",
        data=NewsSourceResponse(items=sources, total=total, page=page, size=size, pages=pages),
    )


@router.get("/{source_id}", response_model=ResponseSchema[NewsSourceItem])
async def get_news_source(source_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return ResponseSchema(message="Berhasil mengambil data news source", data=source)


@router.post("", response_model=ResponseSchema[NewsSourceItem], status_code=status.HTTP_201_CREATED)
async def create_news_source(payload: NewsSourceCreate, db: AsyncSession = Depends(get_db)):
    source = await NewsSourceService.create(db, payload)
    return ResponseSchema(message="News source berhasil dibuat", data=source)
...
async def update_news_source(
    ...
    return ResponseSchema(message="News source berhasil diupdate", data=source)
